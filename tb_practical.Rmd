...
4) Making use of concepts from your previous lectures, can you think of a simple mathematical expression for the endemic equilibrium in terms of the basic reproduction number ($R_0$) ? ...  
...
To proceed with roling out the interventions in our model we need a few more steps to get our functions ready to be called when necessary. 
...
Also, we need a function that we will run our new instance of the model and get plots for us. 
...
Let's create function handles for the basic functions.
...
data0<-get_intervention(sfin, params_new, params_old,times_new,
                             t.interv, fx_scale, int_name, NA) 

```

As we can see from the plot above, we have included dashed lines for WHO's End TB goal (black) and a TB elimination threshold (red).

This, so we can check how our model performs against these reference goals. WHO has define the End TB goal as a 95% reduction in TB incidence by 2035. Elimination is defined as a threshold <1 case per million population. The latter should be interpreted as the theoretical limit for erradication, while the End TB is meant to be an achievable goal, which, if achieved, should leave countries at the door of elimination.     
...
data1<-get_intervention(sfin, params_new, params_old,times_new,
                             t.interv, fx_scale, int_name, data0) 
```
...
data2<-get_intervention(sfin, params_new, params_old,times_new,
                             t.interv, fx_scale, int_name, data1) 
...
```{r echo=FALSE,, warning=FALSE}
## Simulation 2
# An Intervention simulating transm reduction demand generation
int_name   <- "Demand generation"
...
data3<-get_intervention(sfin, params_new, params_old,times_new,
                             t.interv, fx_scale, int_name, data2) 
...
data4<-get_intervention(sfin, params_new, params_old,times_new,
                             t.interv, fx_scale, int_name, data3) 
